...
angular.module('m.data.table')
  ...
  .factory('$mEditDialog', mEditDialog);
...
function mEditDialog($compile, $controller, $document, $mUtil, $q, $rootScope, $templateCache, $templateRequest, $window) {
  ...
  function build(template, options) {
    ...
    var backdrop = $mUtil.createBackdrop(scope, 'm-edit-dialog-backdrop');
    var controller;
    ...
    body.prepend(backdrop).append(element.addClass('m-whiteframe-1dp'));

    positionDialog(element, options.targetEvent.target);

    if(options.focusOnOpen) {
      var autofocus = $mUtil.findFocusTarget(element);
  ...
  function disableScroll(element) {
    var restoreScroll = $mUtil.disableScrollAround(element, body);
  ...
  function positionDialog(element, target) {
    var table = angular.element(target).controller('mCell').getTable();
    ...
    var getTableBounds = function () {
      ...
      if(parent.prop('tagName') === 'M-TABLE-CONTAINER') {
  ...
  function preset(size, options) {
    ...
    return {
      ...
      template:
        '<m-edit-dialog>' +
          '<div layout="column" class="m-content">' +
            '<div ng-if="size === \'large\'" class="m-title">{{title || \'Edit\'}}</div>' +
            '<form name="editDialog" layout="column" ng-submit="submit(model)">' +
              '<m-input-container m-no-float>' +
                '<input name="input" ng-model="model" m-autofocus placeholder="{{placeholder}} "' + getAttrs() + '>' +
            ...
            '</form>' +
          '</div>' +
          '<div ng-if="size === \'large\'" layout="row" layout-align="end" class="m-actions">' +
            '<m-button class="m-primary" ng-click="dismiss()">{{cancel}}</m-button>' +
            '<m-button class="m-primary" ng-click="submit()">{{ok}}</m-button>' +
          '</div>' +
        '</m-edit-dialog>'
    };
  }
  ...
  this.show = function (options) {
    ...
    if(!options.targetEvent.target.classList.contains('m-cell')) {
...
mEditDialog.$inject = ['$compile', '$controller', '$document', '$mUtil', '$q', '$rootScope', '$templateCache', '$templateRequest', '$window'];